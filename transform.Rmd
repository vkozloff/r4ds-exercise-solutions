...
Similarly, the flights that left earliest relative to the scheduled departure time can be found by sorting `dep_delay` in ascending order.
```{r}
...
The flight that left at the earliest time of day can be found by sorting `dep_time` in ascending order.
```{r}
arrange(flights, dep_time)
```
